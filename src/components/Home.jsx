...
const Home = ({ userEmail, uid }) => {
  ...
  useEffect(() => {
    async function fetchNotes() {
      const fetchedNotes = await getDocOrCreateDoc(
        userEmail !== null ? userEmail : userEmail == null
      );
  ...
  return (
    <>
      <motion.div variants={variantPage} initial="hidden" animate="visible">
        <chakra.div
          className="mobile-nav-bar"
          w={{ lg: 36, base: "100vw" }}
          position={{ lg: "absolute" }}
          h={{ lg: "100vh", base: "10vh" }}
          border="1px"
          ...
          <Box display={{ lg: "block", base: "none" }}>
            ...
            >
              <Button
                ...
                bg="var(--icon-color)"
                color="var(--bg-color)"
                _hover={{ background: "var(--icon-shadow-color)" }}
                ml={2}
        ...
        {notes ? (
          <NotesList notes={notes} setNotes={setNotes} userEmail={userEmail} />
        ) : (
          <Spinner
            thickness="5px"
            speed="0.65s"
